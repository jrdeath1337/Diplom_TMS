...
echo "========================================"
echo "Добавьте следующие значения в Settings → Secrets → Actions:"
echo ""
echo "YC_CLOUD_ID:     $(terraform -chdir=terraform output -raw yc_cloud_id)"
echo "YC_FOLDER_ID:    $(terraform -chdir=terraform output -raw yc_folder_id)"
echo "YC_REGISTRY_ID:  $REGISTRY_ID"
echo "YC_CLUSTER_ID:   $(terraform -chdir=terraform output -raw k8s_cluster_id)"
echo "INGRESS_IP:      $(kubectl get svc -n nginx nginx-ingress-ingress-nginx-controller -o jsonpath='{.status.loadBalancer.ingress[0].ip}')"
echo ""
